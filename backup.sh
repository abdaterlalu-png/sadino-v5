...
OUT=${1:-$PWD/backups/sadino_$STAMP}
mkdir -p "$OUT"
...
[[ -n "$DB_ID" ]] || { echo "[FAIL] Container database SADINO aktif tidak ditemukan." >&2; exit 1; }
...
[[ -n "$DB_NAME" && -n "$DB_USER" && -n "$DB_PASS" ]] || { echo "[FAIL] Environment database tidak lengkap." >&2; exit 1; }
docker exec "$DB_ID" mariadb-dump -u"$DB_USER" -p"$DB_PASS" --single-transaction --routines --triggers --events --no-tablespaces "$DB_NAME" | gzip -9 > "$OUT/sadino.sql.gz"
if [[ -n "$APP_ID" ]]; then
